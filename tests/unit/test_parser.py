...
from src.ttgdtparser.exc.http import EndpointNotFoundException, ResourceUnavailableException, WebsiteDomainMovedException, WebsiteDownException
...
class TestParserParseLessons():
    ...
    @pytest.mark.parametrize('group,date,expected', [
        ...
        ('121,123,132', datetime(2025, 10, 15), 
            [
                Lesson(index='1', discipline='Организация пассажирских перевозок и обслуживание пассажиров (по видам транспорта)', \
    ...
    @pytest.mark.asyncio
    async def test_parse_lessons_per_group_and_date(self, parser: LessonTableParser, group: str, date: datetime, expected: list[Lesson]):
    ...
    @pytest.mark.usefixtures('parser')
    @pytest.mark.asyncio
    async def test_parse_lessons_for_unsupported_date(self, parser: LessonTableParser, group: str, date: datetime,expected: list[Lesson]):
...
class TestParserParseChanges(): #Общая дата в html-файле: 3 октября 2025, 2 октября 2025
    ...
    @pytest.mark.usefixtures('parser')
    @pytest.mark.asyncio
    async def test_parse_changes_per_group(self, parser: ChangesTableParser, group: str, expected: list[Change] = changes_expected):
...
class TestParserAddictions():
    ...
    @pytest.mark.parametrize('date,expected', [
        ...
        (datetime(2025, 10, 21), #вторник
            {
                "251-П, 253-П": [],
                "551-П": [Lesson(index=4, by_even_weeks=True, teacher="Гаврилова Е.А.", room="612", date=datetime(2025, 10, 21), discipline="Математика")],
                "141,152 143,154": [Lesson(index=0, by_even_weeks=None, teacher="Криворотова М.В.", room="401", date=datetime(2025, 10, 21), discipline="Введение в теорию решения изобретательских задач, в т.ч. основы инженерных расчетов")],
                "241-П": [Lesson(index=0, by_even_weeks=None, teacher="Архипова Ю.В.", room="405", date=datetime(2025, 10, 21), discipline="Введение в теорию решения изобретательских задач, в т.ч. основы инженерных расчетов")],
                "541-П": [Lesson(index=4, by_even_weeks=None, teacher="Трифонова Е.Г.", room="604", date=datetime(2025, 10, 21), discipline="Введение в теорию решения изобретательских задач, в т.ч. основы инженерных расчетов")],
                "231-П": [
                    Lesson(index=4, by_even_weeks=False, teacher="Архипова Ю.В.", room="405", date=datetime(2025, 10, 21), discipline="Перегонные системы автоматики"),
                    Lesson(index=4, by_even_weeks=True, teacher="Архипова Ю.В.", room="405", date=datetime(2025, 10, 21), discipline="Линии автоматики и телемеханики"),
                ],
            }
        )
    ])
